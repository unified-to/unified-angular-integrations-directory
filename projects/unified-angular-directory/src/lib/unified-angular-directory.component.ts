...
type TIntegrationCategoryType = Exclude<
  ...
  template: `
    <div class="unified">
      ...
      <div class="unified_vendors">
        <a
          *ngFor="let integration of filter(INTEGRATIONS)"
        ...
        >
          ...
          <div class="unified_vendor_inner">
            ...
            <div *ngIf="!nocategories" class="unified_vendor_cats">
              <span
                *ngFor="
                  let cat of integration.categories
                    | filterCategories : CATEGORIES
                "
                >{{ CATEGORY_MAP[cat] }}</span
              >
            </div>
          </div>
        </a>
        <div *ngIf="!filter(INTEGRATIONS).length">
...
export class UnifiedDirectory implements OnInit, OnChanges {
  ...
  @Input() CATEGORY_MAP: { [p in TIntegrationCategory]: string } = {
    crm: 'CRM',
    martech: 'Marketing',
    ticketing: 'Ticketing',
    ...
    return (
      integrations?.filter(
        (integration) =>
          ...
          integration.categories.includes(this.selectedCategory)
      ) || []
    );
  }
  ...
  unified_get_auth_url(integration: IIntegration) {
    ...
    url += `&success_redirect=${encodeURIComponent(
      this.success_redirect || window.location.href
    )}`;
    // }
    // if (this.failure_redirect) {
    url += `&failure_redirect=${encodeURIComponent(
      this.failure_redirect || window.location.href
    )}`;
    // }
    return url;
  }
  ...
  async setup() {
    ...
    const url = `${APIURL}/unified/integration/workspace/${
      ...
      this.environment === 'Production' || !this.environment
        ? ''
        : '&env=' + encodeURIComponent(this.environment)
    }`;
